...
http.open(options.method,url,options.async,options.user,options.password);if(options.method==='POST'&&Object.keys(options.files).length){var formData=new FormData();for(var keyFile in options.files){if(options.files.hasOwnProperty(keyFile)){formData.append('files[]',options.files[keyFile])}}
for(var keyData in options.data){if(options.data.hasOwnProperty(keyData)){formData.append(keyData,options.data[keyData])}}
query=formData}
else{http.setRequestHeader('Content-Type',options.contentType);if(options.method==='GET'||options.method==='DELETE'){query=null}}
