...
return this.replace(new RegExp('/^'+regex+'|'+regex+'$/gm'),'')};String.prototype.selectorReplaceId=function(){return this.replace(/#([-\w]+)/g,"[id=$1]")};Math.rand=function(min,max){return this.floor(this.random()*(max-min+1))+min};this.querySelectorAll=function(selector){var _tmp=new jNet.jNArray();var _arr=document.querySelectorAll(selector.selectorReplaceId());for(var i=0;i<_arr.length;++i){_tmp.push(new jNet.jNDocQuery(_arr[i]))}
