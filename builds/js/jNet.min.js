...
http.onreadystatechange=options.stateChange;http.onload=options.success;http.onerror=options.fail;http.onprogress=options.progress;var url;if(options.method=="GET"){url=options.url+"?"+this.serialize(options.data)}
else{url=options.url}
http.open(options.method,url,options.async);if(options.method=="POST"&&Object.keys(options.files).length){var formData=new FormData();for(var keyFile in options.files){if(options.files.hasOwnProperty(keyFile)){formData.append('files[]',options.files[keyFile])}}
for(var keyData in options.data){if(options.data.hasOwnProperty(keyData)){formData.append(keyData,options.data[keyData])}}
http.send(formData);}
else{http.setRequestHeader('Content-Type',options.contentType);if(options.method=="GET"){http.send()}
else{http.send(this.serialize(options.data))}}
return http};return this})()