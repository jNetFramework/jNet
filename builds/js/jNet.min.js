...
http.onreadystatechange=options.stateChange;http.onload=options.success;http.onerror=options.fail;http.onprogress=options.progress;var url;var query=this.serialize(options.data);if(query&&(options.method==='GET'||options.method==='DELETE')){if(options.url.indexOf('?')===-1){options.url+='?'+query}
...
http.send(formData);}
else{http.setRequestHeader('Content-Type',options.contentType);if(options.method==='GET'||options.method==='DELETE'){http.send()}
else{http.send(query)}}
